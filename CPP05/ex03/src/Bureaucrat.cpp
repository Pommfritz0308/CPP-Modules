...
{
	os << GBOLD(b.getName()) << GBOLD(", bureaucrat grade ") << BOLD(b.getGrade()) << std::endl;
	return os;
}
...
{
	if (f.getSigned())
		std::cout << GBOLD("Bureaucrat \"") <<  GBOLD(_name) <<  GBOLD("\" signed ")
					<<  GBOLD(f.getName()) << std::endl;
	else
		std::cout << RBOLD("Bureaucrat \"") << RBOLD(_name) << RBOLD("\" couldn't sign ")
					<< RBOLD(f.getName()) << RBOLD(" because the grade was too low") << std::endl;
}
