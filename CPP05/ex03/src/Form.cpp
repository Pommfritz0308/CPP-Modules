...
{
	os << GBOLD(f.getName()) << GBOLD(" form status: ") << BOLD(BOOL_TO_STRING(f.getSigned()))
...
{
	std::cout << GBOLD("Bureaucrat ") << GBOLD(executor.getName()) << GBOLD(" executes ") << GBOLD(this->getName()) << std::endl;
	if (!_signed)
